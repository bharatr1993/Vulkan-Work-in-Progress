...
InitGraphics( )
{
	...
	Init12SpirvShader( "myvert.spv", &ShaderModuleVertex );
	Init12SpirvShader( "myfrag.spv", &ShaderModuleFragment );
...
Init13DescriptorSetLayouts( )
{
	...
	VkDescriptorSetLayoutBinding		MatrixSet;
		...
		MatrixSet.stageFlags         = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
...
Init13DescriptorSets( )
{
	...
	VkDescriptorSetAllocateInfo			vdsai;
		...
		vdsai.pSetLayouts = &DescriptorSetLayouts[0];
...
Init14GraphicsVertexFragmentPipeline( VkShaderModule vertexShader, VkShaderModule fragmentShader, VkPrimitiveTopology topology, OUT VkPipeline *pGraphicsPipeline  )
{
	...
	result = vkCreatePipelineLayout( LogicalDevice, IN &vplci, PALLOCATOR, OUT &GraphicsPipelineLayout );//SuccessFull!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	REPORT( "vkCreatePipelineLayout" );
...
VK_SHADER_STAGE_ALL
#endif
		vpssci[0].module = ShaderModuleVertex;
		vpssci[0].pName = "main";
		...
		vpssci[1].module = ShaderModuleFragment;
		vpssci[1].pName = "main";
...
VK_FORMAT_X = VK_FORMAT_R32_SFLOAT
	...
	VkPipelineVertexInputStateCreateInfo			vpvisci;			// used to describe the input vertex attributes
		...
		vpvisci.vertexAttributeDescriptionCount = 1;
...
RenderScene( )
{
	...
	VkClearColorValue			vccv; ////Background Color!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		vccv.float32[0] = 0.0;
		vccv.float32[1] = 0.0;
		vccv.float32[2] = 0.0;
		vccv.float32[3] = 1.0;
	...
	vkCmdBindDescriptorSets( CommandBuffers[nextImageIndex], VK_PIPELINE_BIND_POINT_GRAPHICS, GraphicsPipelineLayout, 0, 2, &DescriptorSets[0], 0, (uint32_t *)nullptr );//Modify Descriptor Set Count!!!
