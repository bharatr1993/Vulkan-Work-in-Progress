...
layout( std140, set = 12, binding = 0 ) uniform matBuf
{
        mat4 uModelMatrix[100];
        mat4 uViewMatrix;
...
main( ) 
{
	...
	mat4  M = Matrices.uModelMatrix[0];
	M[0][0] *= 2.;
	M[1][1] *= 2.;
	...
	vN = normalize( mat3( Matrices.uNormalMatrix[1] ) * aNormal );
	...
	#ifdef DO_INSTANCING
		int    NUMINSTANCES = Matrices.count;
				
		P = Matrices.uProjectionMatrix;
		M = Matrices.uModelMatrix[gl_InstanceIndex];
		V = Matrices.uViewMatrix;
		VM = V * M;
		PVM = P * VM;

		gl_Position = PVM * vec4( aVertex, 1. + 0.001*gl_InstanceIndex );
	#endif
